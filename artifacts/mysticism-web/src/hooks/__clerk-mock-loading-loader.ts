 ...
 * and returns a mock with useUser returning { isSignedIn: undefined, isLoaded: false }.
 *
 * This simulates the "Clerk still loading" state where the short-circuit
 * must NOT fire and the request delegates to useSSEChat normally.
 *
...
export async function resolve(
  ...
  if (specifier === "@clerk/react" || specifier.startsWith("@clerk/react/")) {
    return {
      shortCircuit: true,
      url: "clerk-react-loading-mock://mock",
    };
  }
...
export async function load(
  ...
  if (url === "clerk-react-loading-mock://mock") {
    return {
      shortCircuit: true,
      format: "module" as const,
      source: `
        export function useUser() {
          return { isSignedIn: undefined, isLoaded: false, user: null };
        }
